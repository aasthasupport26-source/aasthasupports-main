...
function AdminTemples() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this temple? This may fail if there are pujas attached to it.')) return;
    try {
          ...
          <tbody className="divide-y divide-gold/10">
            {temples.map((temple) => (
              <tr key={temple.id} className="hover:bg-cream/50 transition">
                <td className="p-3">
                  {temple.image_url ? (
                    <img src={temple.image_url} alt={temple.name} className="w-10 h-10 rounded-md object-cover" />
                  ) : (
                ...
                <td className="p-3 text-sm text-muted-foreground">{temple.city}{temple.state ? `, ${temple.state}` : ''}</td>
                <td className="p-3">
                  <span className={`px-2 py-1 text-[10px] font-bold uppercase rounded-full tracking-wider ${temple.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                    {temple.active ? 'Active' : 'Inactive'}
                  </span>
                </td>
                <td className="p-3 text-right space-x-2">
                  <button onClick={() => handleEdit(temple)} className="p-1.5 text-gold hover:bg-gold/10 rounded transition" title="Edit">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(temple.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded transition" title="Delete">
