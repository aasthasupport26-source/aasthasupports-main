...
import { getAdminTemples, getAdminPujas, createPuja, updatePuja, deletePuja, getAdminPackages, createPackage, updatePackage, deletePackage } from '@/lib/admin.functions';
...
function AdminPujas() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Are you sure you want to delete this puja? All its packages will be lost!')) return;
    try {
              ...
              <tr key={puja.id} className="hover:bg-cream/50 transition">
                <td className="p-3">
                  <div className="flex items-center gap-3">
                    {puja.image_url ? (
                      <img src={puja.image_url} alt={puja.name} className="w-10 h-10 rounded-md object-cover" />
                    ) : (
                      <div className="w-10 h-10 rounded-md bg-cream border border-gold/20 flex items-center justify-center text-xs text-muted-foreground">No img</div>
                    )}
                    <div>
                ...
                <td className="p-3 text-sm text-muted-foreground">{puja.temple?.name || 'Unknown'}</td>
                <td className="p-3 text-sm text-muted-foreground">{puja.duration_minutes ? `${puja.duration_minutes}m` : '-'}</td>
                <td className="p-3">
                  <span className={`px-2 py-1 text-[10px] font-bold uppercase rounded-full tracking-wider ${puja.active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                    {puja.active ? 'Active' : 'Inactive'}
                  </span>
                </td>
                <td className="p-3 text-right space-x-2">
                  <button onClick={() => setManagingPackagesFor(puja)} className="p-1.5 text-royal hover:bg-royal/10 rounded transition" title="Manage Packages">
                    <PackagePlus className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleEdit(puja)} className="p-1.5 text-gold hover:bg-gold/10 rounded transition" title="Edit">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(puja.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded transition" title="Delete">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-royal w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center p-5 border-b border-gold/20 bg-cream">
              ...
              <button onClick={handleCancel} className="p-1 text-muted-foreground hover:text-maroon-deep transition">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 space-y-4 overflow-y-auto flex-1">
              <div>
                ...
                <select className="w-full border border-gold/30 rounded-md p-2 text-sm focus:ring-1 focus:ring-gold outline-none"
                  value={formData.temple_id || ''} onChange={(e) => setFormData({ ...formData, temple_id: e.target.value })}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" className="w-full border border-gold/30 rounded-md p-2 text-sm focus:ring-1 focus:ring-gold outline-none"
                    value={formData.name || ''} onChange={(e) => setFormData({ ...formData, name: e.target.value, slug: isCreating ? e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, '-') : formData.slug })} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-maroon-deep mb-1">URL Slug *</label>
                  <input type="text" className="w-full border border-gold/30 rounded-md p-2 text-sm focus:ring-1 focus:ring-gold outline-none bg-gray-50"
                    value={formData.slug || ''} onChange={(e) => setFormData({ ...formData, slug: e.target.value })} />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-maroon-deep mb-1">Description</label>
              ...
              <div className="flex items-center gap-2 pt-2">
                ...
                <label htmlFor="active" className="text-sm text-maroon-deep cursor-pointer">Active / Visible on site</label>
              </div>
            </div>
            <div className="p-5 border-t border-gold/20 flex justify-end gap-3 bg-gray-50">
              <button onClick={handleCancel} className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-maroon-deep transition">
                Cancel
              </button>
              <button onClick={handleSave} className="flex items-center gap-2 bg-maroon-deep text-cream px-5 py-2 rounded-md text-sm font-medium hover:bg-maroon transition shadow-md">
...
function PackagesManagerModal({ puja, onClose }: { puja: any, onClose: () => void }) {
  ...
  useEffect(() => { loadPackages(); }, [puja.id]);
    ...
    <div className="fixed inset-0 bg-black/60 z-[60] flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-royal w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
        <div className="flex justify-between items-center p-5 border-b border-gold/20 bg-cream">
          ...
          <button onClick={onClose} className="p-1 text-muted-foreground hover:text-maroon-deep transition">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-5 flex-1 overflow-y-auto">
          {editingId ? (
            <div className="border border-gold/30 p-4 rounded-xl bg-gray-50 mb-6">
              <h4 className="font-medium text-maroon-deep mb-3">{isCreating ? 'New Package' : 'Edit Package'}</h4>
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs font-medium text-maroon-deep mb-1">Name *</label>
                    <input type="text" className="w-full border border-gold/30 rounded-md p-2 text-sm"
                      value={formData.name || ''} onChange={(e) => setFormData({ ...formData, name: e.target.value })} placeholder="e.g. Basic, Premium" />
                  </div>
                  <div>
                    <label className="block text-xs font-medium text-maroon-deep mb-1">Price (₹) *</label>
                    <input type="number" className="w-full border border-gold/30 rounded-md p-2 text-sm"
                ...
                <div className="flex items-center gap-2 pt-1">
                  <input type="checkbox" id="pkg_active" checked={formData.active}
                    onChange={(e) => setFormData({ ...formData, active: e.target.checked })} />
                  <label htmlFor="pkg_active" className="text-sm text-maroon-deep cursor-pointer">Active</label>
                </div>
                <div className="flex justify-end gap-2 pt-2">
                  <button onClick={() => setEditingId(null)} className="px-3 py-1.5 text-xs font-medium text-muted-foreground hover:text-maroon-deep">Cancel</button>
                  <button onClick={handleSave} className="bg-gold text-maroon-deep px-4 py-1.5 rounded-md text-xs font-medium shadow-sm hover:bg-gold-soft">Save Package</button>
                </div>
              </div>
            </div>
          ) : (
            <button 
              onClick={() => {
                setEditingId('new');
                setIsCreating(true);
                setFormData({ puja_id: puja.id, name: '', description: '', price: 0, active: true });
              }}
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {packages.map(pkg => (
                <div key={pkg.id} className="flex items-center justify-between p-4 border border-gold/20 rounded-xl hover:bg-cream/30 transition">
                  <div>
                    <div className="font-medium text-maroon-deep">{pkg.name} <span className="text-sm text-maroon ml-2">₹{pkg.price.toLocaleString('en-IN')}</span></div>
                    <div className="text-xs text-muted-foreground mt-1">{pkg.description || 'No description'}</div>
                    {!pkg.active && <span className="text-[10px] bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full mt-1 inline-block">Inactive</span>}
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => { setEditingId(pkg.id); setFormData(pkg); setIsCreating(false); }} className="p-1.5 text-gold hover:bg-gold/10 rounded">
                      <Edit2 className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(pkg.id)} className="p-1.5 text-red-500 hover:bg-red-50 rounded">
                      <Trash2 className="w-4 h-4" />
                    </button>
                  </div>
                </div>
              ...
              {packages.length === 0 && !editingId && (
                <p className="text-center text-sm text-muted-foreground p-8 bg-gray-50 rounded-xl border border-gray-100">No packages created yet.</p>
              )}
            </div>
          )}
        </div>
